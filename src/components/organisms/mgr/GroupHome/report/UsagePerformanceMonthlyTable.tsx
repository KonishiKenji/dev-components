...
import UsagePerformanceReportMonthlyCell from "@components/organisms/mgr/GroupHome/report/cells/UsagePerformanceReportMonthlyCell";
import { REPEAT_MONTHLY, ReportState } from "@stores/domain/report/type";
...
const UsagePerformanceMonthlyTable = (props: Props): JSX.Element => {
  const tableHead = reportMonthlyHeader(
    props.isEditing,
    props.facility.nightSupportFlag,
    props.classes
  );
  const rows = props.isEditing
    ? props.report.reportMonthly.after
    : props.report.reportMonthly.before;
...
const mapDispatchToProps = (dispatch: Dispatch): DispatchProps => {
  ...
  return {
    setStatusType: (
      ...
      reportDispatches.setStatusType(event.target.value, key, REPEAT_MONTHLY),
    setNightSupportType: (
      event: React.ChangeEvent<HTMLInputElement>,
      key: number
    ): void =>
    ...
    setLifeSupportFlg: (
      ...
      reportDispatches.setLifeSupportFlg(
        event.target.value,
        key,
        REPEAT_MONTHLY
      ),
    setHomeCareFlg: (
      ...
      reportDispatches.setHomeCareFlg(event.target.value, key, REPEAT_MONTHLY),
    setRemarks: (remarks: string, key: number): void =>
      reportDispatches.setRemarks(remarks, key, REPEAT_MONTHLY)
  };
};
