...
import RecordWorkOutOfFacility from "@components/organisms/mgr/IAB/record/RecordWorkOutOfFacility";
...
import initialValues, {
  RecordMonthlyValues
} from "@initialize/mgr/IAB/record/monthly/initialValues";
import validation from "@initialize/mgr/IAB/record/monthly/validation";
...
import * as IABReportActions from "@stores/domain/mgr/IAB/report/actions";
 ...
 * 業務日誌
 */
const MonthlyOperationDiary: React.SFC<Props> = (props: Props) => {
  ...
  const confirmDiscardFormChanges = (nextValues: RecordMonthlyValues): void => {
  ...
  const validate = (values: RecordMonthlyValues): object | void => {
  ...
  return (
    ...
    >
              ...
              <Form>
                ...
                <div className={props.classes.usageSituation}>
                  ...
                  {props.user.facility_type === FacilityType.IKOU && (
                    <UsageSituationBox
                      label="移行準備支援体制加算 I"
                      value={props.operation.counts.numberOfSupportIkou1}
                      unit={`/${props.operation.counts.numberOfUsers}人`}
                    />
                  )}
                  {/* 施設外支援の項目を出す（A/B型のみ） */}
                  {(props.user.facility_type === FacilityType.A ||
                    ...
                    <UsageSituationBox
                      label="施設外支援"
                      value={
                        props.operation.counts.numberOfSupportOutOfFacility
                      }
                      unit={`/${props.operation.counts.numberOfUsers}人`}
                    />
                  )}
                ...
                <div className={props.classes.recordWrapper}>
                  ...
                  <RecordWorkOutOfFacility
                    formikProps={formikProps}
                    isEditable={props.isEditing}
                    name="operation[0].workplace_company"
                    label="施設外就労同行職員"
                    defaultValue="-"
                    workplace_company={props.operation.workplace_company}
                    staffs={props.staffOptions}
                  />
                </div>
      ...
      {isOpenDialog && (
        <InOutReportModal
          countsPerStatus={props.reportState.IABSummary.countsPerStatus}
          inOutRecords={props.reportState.IABSummary.inoutRecords}
          isOpen={isOpenDialog}
          onClose={closeDialog}
          targetName=""
          date={date}
...
const mapDispatchToProps = (dispatch: Dispatch): DispatchProps => {
  ...
  const reportDispacher = IAB.reportDispatcher(dispatch);
  ...
  return {
    ...
    unsetSummary: (): { type: string } =>
      dispatch(IABReportActions.unsetIABDailySummary()),
