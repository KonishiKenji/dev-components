import { BasicErrors } from "@interfaces/mgr/JIRITSUKUNRENSEIKATSU/Facility/basic";
import { SubtractionItemErrors } from "@interfaces/mgr/JIRITSUKUNRENSEIKATSU/Facility/subtractionItem";
import { AdministrationErrors } from "@interfaces/mgr/JIRITSUKUNRENSEIKATSU/Facility/administration";
import { AdditionalItemErrors } from "@interfaces/mgr/JIRITSUKUNRENSEIKATSU/Facility/additionItem";
...
const AdditionalValidation = (values: FacilityValues): AdditionalItemErrors => {
  return {
    additionalItem: {
      continuationPersonLastYear: validateSwitcher(
        values.additionalItem.employmentTransitionSupportFlag,
        validator(values.additionalItem.continuationPersonLastYear, {
          type: "selectRequired",
          value: DEFAULT_SELECT_VALUE
        })
      ),
      numberOfContinuators: validateSwitcher(
        values.additionalItem.employmentTransitionSupportFlag &&
          values.additionalItem.continuationPersonLastYear === "2",
        validator(
          values.additionalItem.numberOfContinuators,
          "required",
          "naturalNumberNonZero",
          {
            type: "lowerLimit",
